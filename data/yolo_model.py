...
class YOLO:
    def __init__(self, path_to_model, obj_threshold, nms_threshold):
        """Init.
        ...
        self._yolo = load_model(path_to_model)
    ...
    def _process_feats(self, out, anchors, mask):
        ...
        anchors_tensor = np.reshape(np.array(anchors),
                                   (1, 1, len(anchors), 2))
        # print("out shape: {}, type: {}".format(out.shape, type(out)))
        # print("anchors_tensor type:{}, shape:{}".format(type(anchors_tensor), (anchors_tensor.shape)))
        out = out[0]
        
        # biox_xy = K.get_value(K.sigmoid(out[..., :2]))
    ...
    def _yolo_out(self, outs, shape):
        ...
        for out, mask in zip(outs, masks):
            ...
            # print("process feats: {}, filter boxes: {}".format((t2 - t1), (datetime.now() - t2)))
            boxes.append(b)
            classes.append(c)
            scores.append(s)
        # print("finish processing and filtering: {}".format(datetime.now() - t0))
